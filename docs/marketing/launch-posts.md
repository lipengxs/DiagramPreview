# DiagramPreview Promotion Kit

This folder splits the launch and comment materials into smaller files so each channel can be copied, edited, and posted independently.

## Files

- [positioning.md](./positioning.md): core positioning, audience, and message angles.
- [low-barrier-channels.md](./low-barrier-channels.md): low-friction websites and where to post.
- [zh-launch-posts.md](./zh-launch-posts.md): Chinese launch posts for V2EX, 掘金, 博客园, SegmentFault, CSDN, 知乎, 开源中国.
- [en-launch-posts.md](./en-launch-posts.md): English launch posts for Hacker News, Dev.to, Hashnode, Indie Hackers, Medium, Product Hunt.
- [comment-templates.md](./comment-templates.md): natural comment templates for Chinese and English communities.
- [platform-playbook.md](./platform-playbook.md): channel-specific posting strategy and anti-spam notes.
- [next-tool-ideas.md](./next-tool-ideas.md): future tool ideas that fit the current site theme.

## Recommended First Wave

1. V2EX: publish as a sincere side-project feedback post.
2. 掘金: publish as a practical article about AI-generated Mermaid preview.
3. 博客园 or CSDN: publish SEO-style long-tail article.
4. Dev.to or Hashnode: publish English tutorial-style article.
5. Hacker News Show HN: post only after the homepage and first tools feel polished enough for direct trial.

## Core Link

https://diagrampreview.com
