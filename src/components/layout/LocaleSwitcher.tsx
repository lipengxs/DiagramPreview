    ...
    <DropdownMenu.Root>
      ...
      <DropdownMenu.Portal>
        <DropdownMenu.Content
          align="end"
          className="z-50 max-h-80 w-44 overflow-auto rounded-md border border-slate-200 bg-white p-1 text-sm text-slate-800 shadow-xl"
        >
          {locales.map((candidate) => (
            <DropdownMenu.Item
              key={candidate}
              className={cn(
                "flex cursor-pointer items-center justify-between rounded px-2 py-2 outline-none hover:bg-slate-100",
